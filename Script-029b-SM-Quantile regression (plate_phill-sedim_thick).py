...
def fit_model(q):
    res = mod.fit(q=q)
    return [q, res.params['Intercept'], res.params['plate_phill']] +             res.conf_int().loc['plate_phill'].tolist()
...
plt.title("Mariana Trench: Quantile regression \nof sediment thickness at Phillipine Plate by 25 bathymetric profiles", fontsize=14)
